...
struct Table_fields {
	...
	std::vector<std::string> name_of_colums;
	std::string name_of_table;
	std::vector<std::string> date;

	std::vector<std::string> search_name_of_colums;
...
public:
	...
	virtual std::string BuildQuery() {
		...
		fields.select_query = "SELECT " + count_cols(fields.name_of_colums) + " FROM " + fields.name_of_table +  " WHERE " + search_name_of_cols(fields.search_name_of_colums, fields.date) + "; ";
